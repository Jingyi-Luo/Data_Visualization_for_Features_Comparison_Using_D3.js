...
#
n_use = 200
X_test_true = X_test.iloc[true_index[:n_use]]
X_test_false = X_test.iloc[false_index[:n_use]]
X_test_use = X_test_true.append(X_test_false)
#
#y_test = pd.DataFrame(y_test)
#y_test.columns = ['y']
pca = PCA(n_components=2)
train_features = list(X_test_use.columns)
train_features.remove('y')
pca.fit(X_test_use[train_features])
pc_list = pca.transform(X_test_use[train_features])
X_test_use['PC1'] = pc_list[:,0]
X_test_use['PC2'] = pc_list[:,1]
X_test_use.to_csv("X_test_use.csv", sep=',')
